...
const ChatInterface = ({ selectedChat }) => {
    ...
    useEffect(() => {
        ...
        socket.on("message recieved", (newMessageRecieved) => {
            if (!selectedChat || selectedChat._id !== newMessageRecieved.chat._id) {
                // Give notification (not implemented here)
            } else {
    ...
    const fetchMessages = async () => {
        ...
        try {
            ...
            const res = await axios.get(`http://localhost:5000/api/chat/${selectedChat._id}/messages`, {
    ...
    const getOtherUserName = () => {
        return selectedChat.participants[0]._id === user._id ? selectedChat.participants[1].name : selectedChat.participants[0].name;
    };
        ...
        <div className="d-flex flex-column h-100">
            ...
            <div className="p-3 border-top bg-white">
                <InputGroup>
                    ...
                    />
                    <Button variant="primary" onClick={sendBtnClick}>Send</Button>
                </InputGroup>
            </div>
        </div>
    );
